...
void Isotp_Listener::send_cf_telegram()
{
  ...
  actual_cf_count = ++actual_cf_count & 15;
  int nr_of_bytes = 1;
  ...
  // do we always need 8 Bytes? Test it..
...
/*
copies the data into the send buffer and does the asynchronous send
*/
...
int Isotp_Listener::eval_msg(int can_id, unsigned char data[8], int len)
{
  ...
  if (frametype == FrameType::First)
  {
    ...
    read_from_can_msg(data, 2, dl > 6 ? 6 : dl); // spec. violation, but a first frame could contain only a short msg, so check the dl here
  ...
  if (frametype == FrameType::FlowControl)
  {
    ...
    { // we use -1 as indicator that there's no block size
  ...
  if (frametype == FrameType::Consecutive)
  {
    ...
    if (actual_state == ActualState::WaitConsecutive)
    {
      ...
      {
        if (actual_receive_pos == expected_receive_buffer_size)
        {
